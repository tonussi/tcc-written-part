...
% estudar colisão, múltiplos proposers concorrentes
% otimizações por causa do coordinator
...
% Algumas das métricas avaliadas foram Saturação da vazão (k requisições/s), Vazão média (k requisições/s), Percentil 90 da Latência (ms).

% \subsection{AS LIMITAÇÕES DA ABORDAGENS DO TRABALHO}

% Os resultados de \textcite{renan2021hermes} mostram comparações das curvas de Latência (percentil 90) por vazão média (k requisições/s),
