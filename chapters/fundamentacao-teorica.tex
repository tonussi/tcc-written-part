...
% O modelo um sistema distribuído, que recebe mensagens do mundo externo, precisa de alguma forma de ordenação das mensagens.
Existem na literatura várias técnicas de ordenação de mensagens. Um ordenador de mensagens é um importante mecanismo para promover coerência, em execução de comandos, entre os participantes de um sistema distribuído. A seguir 
%vamos ver alguns dos 
são apresentados os
principais tipos de ordenação de mensagens.

%\textbf{FIFO}
\subsection{Ordem FIFO}
A ordenação por \textit{First-In First-Out} (\gls{FIFO}), que significa o primeiro a entrar é o primeiro a sair, 
%é definida por \textcite{PauloVerissimoLuisRodrigues} como sendo: ``Quaisquer 
estabelece que
%duas 
as mensagens enviadas pelo mesmo participante e entregues para qualquer participante, são entregues na mesma ordem 
%enviada
de envio
\cite{PauloVerissimoLuisRodrigues}.
%.'' 
%Pela própria definição de \textit{FIFO} se usarmos de 2 em 2, apenas, até um certo ponto \textit{FIFO} é útil, mas em determinadas situações, onde temos um terceiro participante e, sendo uma situação mais complexa de troca de mensagens, \textit{FIFO} se torna insuficiente para cumprir com a coerência de natureza temporal, na ordenação, pois as mensagens se tornam embaralhadas ao longo do tempo \cite{PauloVerissimoLuisRodrigues}.
...
%O conceito de ordem total é definido por \textcite{PauloVerissimoLuisRodrigues} como: ``para quaisquer 2 mensagens que são entregues para qualquer par de participantes, são entregues na mesma ordem para ambos os participantes''. 
%Todos 
Em linhas gerais, no critério de ordenação total todos os participantes recebem 
%todas 
as mensagens enviadas na mesma ordem. Porém, em termos mais técnicos, pode-se definir a partir de \textcite{PauloVerissimoLuisRodrigues} que:
\begin{quote}
...
\end{quote}
%, uma vantagem é que sempre temos todos os servidores equivalentes e, sabendo as mensagens na ordem correta, por outro lado temos uma desvantagem, a troca de mensagens cresce muito em função do aumento de servidores e requisições dos clientes podendo acarretar em lentidão de resposta. 
% Há uma motivação do presente trabalho, em dar ênfase para a ordem total, pois %\textcite{PauloVerissimoLuisRodrigues} nos diz que o método da 
% a ordenação total é 
%tipicamente usado para atingir determinismo entre réplicas de processos, em um sistema distribuído e, para assegurar 
% um dos requisitos para a implementação de \gls{RME}, garantindo
%, 
% que diferentes participantes
% (em \gls{RME}, réplicas do serviço) receberão exatamente as mesmas requisições ordenadas.
%evoluirão igualmente, pelos estados da aplicação. A partir do conceito de ordem total é possível criar algoritmos de ordenação, os algoritmos tem o objetivo de garantir que todas as mensagens sejam entregues para todos os destinatários na mesma ordem. 

O termo difusão em ordem total (do Inglês \textit{Total order broadcast}) é também conhecido como difusão atômica (do Inglês \textit{atomic broadcast}), algumas vezes representado pela primitiva \gls{AB-cast}. A seguir são apresentada as propriedades necessárias para assegurar ordenação total e os algoritmos para implementação de protocolo \textit{AB-cast}.
...
O Algoritmo \ref{algo:priviledge-based-simple-destinations} e o Algoritmo \ref{algo:simple-fixed-sequencer-destinations} são iguais e portanto tem a mesma explicação.

\section{Consenso}
\label{sec:consenso}

%Como vimos na seção \ref{sec:ordenacao_mensagens} se temos difusão atômica, por quê usar consenso? Existem vários motivos, mas um bom motivo é a quantidade de servidores distribuídos e a quantidade massiva de requisições dos clientes, ou seja, visto que 
Protocolos de consenso distribuído também podem ser utilizados para implementar entrega totalmente ordenada de requisições \cite{conf/dsn/EkwallS06, MilosevicHutleSchiper2011}. Esses protocolos surgem como uma boa alternativa, visto que a
difusão atômica 
%gera 
pode gerar
uma quantidade muito grande de requisições.
%precisamos de protocolos mais leves e que ainda assim garantam que o sistema evolua de maneira equivalente, consistente e, promova disponibilidade, em outras palavras muito se discute na literatura, que protocolos de consenso podem ser usados para implementar difusão atômica \cite{conf/dsn/EkwallS06, MilosevicHutleSchiper2011}. Mas aplicações em sistemas distribuídos são tipicamente complicadas para se construir, por causa desse fator de consenso entre os participantes. 
Segundo \textcite{lamport2001paxos}, os algoritmos de consenso precisam garantir 3 premissas básicas:
...
O protocolo Raft foi desenvolvido por \textcite{raft}. 
%É um algoritmo usado para obter consenso sobre um determinado valor. 
% Os autores procuraram ser bastante didáticos ao construir esse algoritmo.
Este protocolo espera que exista um sistema de replicação de \textit{logs} em cada instância que executa o protocolo. Raft é assimétrico, pois os candidatos elegem um líder (também conhecido como alternância de liderança). Cada candidato pode ser visto como uma instância do sistema Raft. Existem três estados que um candidato pode alcançar: \textit{líder}, candidato, seguidor.
...
\textcite{kubernetes/docs/2022} é uma ferramenta de orquestração de contêineres inicialmente desenvolvida por engenheiros da Google, que provê formas de fazer a gestão de aplicações em contêineres, abordando diferentes ambientes de implantação, tais como: físicos, virtuais ou, mistos. Quando trata-se de sistemas com replicação, naturalmente busca-se uma infraestrutura robusta que entrega orquestração personalizada e automatizada \cite{Stubbs2015DistributedSO}, baseado em métricas e tolerante a falhas \cite{vayghan2021kubernetes}.

Algumas das principais características do Kubernetes são: prover alta disponibilidade, escalabilidade e recuperação de desastres. O conceito de alta disponibilidade Kubernetes se trata de mecanismos internos que permitem que um \textit{Pod} tenha seus serviços sendo fornecidos continuamente \cite{vayghan2021kubernetes}. A escalabilidade permite expandir ou retrair a quantidade de nós de trabalho no \textit{cluster}, dependendo da carga sob demanda. A recuperação de desastres trata de prover a restauração de \textit{Pods} em casos de falhas. Dentre os vários tipos de componentes do Kubernetes se pode mencionar alguns básicos:
