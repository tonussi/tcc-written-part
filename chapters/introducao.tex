...
% A seguir apresentaremos nosso objetivo geral e os específicos.

% \sout{O objetivo geral se trata de Implementação de Replicação por Máquina de Estados em Arquiteturas de Microsserviços.

% \ct{Você pode discutir um pouco mais o objetivo. Aparece em uma frase clara com o objetivo (sem aspas nem nada, apenas uma frase que explique o objetivo. E, em seguida, pode reforçar o objetivo com detalhes importantes e possíveis contribuições por trás desse objetivo.}
...
%Especificando um pouco mais sobre nossos objetivos, podemos dizer que está no escopo:

\begin{itemize}

%\item Avaliar os itens no capítulo de trabalhos futuros de \textcite{renan2021hermes}.

% \textit{Aprimorar o serviço de ordenação proposto em \textcite{renan2021hermes}:}
%Para isso seguimos mapeando quais melhorias podem ser endereçadas e implementadas.

\item \textit{Meio de comunicação:} Implementar a interface de comunicação do Hermes para possibilitar requisições HTTP.

% Desenvolvimento de estratégias para aumento da vazão na entrega de requisições às réplicas.

%\item Implementar um mecanismos para Replicação por Máquinas de Estado (RME) utilizando arquitetura de microsserviços com base no trabalho proposto por \textcite{renan2021hermes}.

% \item \textit{Implementar Replicação por Máquinas de Estado (\gls{RME}) 
%utilizando arquitetura de 
% como um microsserviço:}
% Este 
%trabalho propõe estender 
% objetivo visa utilizar o
% serviço de ordenação de mensagens
%código Hermes 
% proposto por \textcite{renan2021hermes}
% para implementar um serviço completo de \gls{RME}.
%de um serviço de interceptador independente da aplicação que é responsável pela ordenação de comandos destinados à cada réplica. Após isso desejamos obter resultados através de experimentos. 

% Com esse serviço, espera-se que desenvolvedores de aplicações possam se beneficiar de redundância e maior disponibilidade dos seus serviços sem ter que implementar aspectos específicos de tolerância a falhas.

% \item \textit{Desenvolver um protótipo do serviço \gls{RME} no topo do orquestrador de contêineres Kubernetes:} Este protótipo permitirá observar a aplicabilidade e transparência do serviço na prática e observar os desafios e limitações da configuração da infraestrutura e do serviço. 
...
%simples, 
%ou 
% serviço de travas distribuído ou mesmo a utilização de aplicações desenvolvidas por terceiros, de forma que se beneficiarem do serviço de replicação proposto.
...
% \item Revisão do estado da arte

% Este trabalho também pretende explorar o estado da arte na área de aplicações altamente disponíveis e que garantem estados consistentes das réplicas de uma aplicação.
\end{itemize}
