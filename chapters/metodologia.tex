...
A intercepção é um trabalho de fatores em conjunto, começando pelo Kubernetes e os arquivos YAML que descrevem o \textit{cluster} Kubernetes. Os arquivos \textit{YAML} têm as descrições de Afinidades, Anti-Afinidades, e nodeSelector para atrelar cada \textit{Pod} replicado ao seu nó servidor específico.

\subsection{Implementação da interface \textit{Communicator}}
...
A Figura \ref{fig:componentes-hermes} detalha a declaração das interfaces que compõem o interceptador. Cada elemento abstrato deverá ser concretizado a partir da implementação dos métodos descritos a seguir. O Hermes possui interfaces \textit{communicator} e \textit{orderer}, com base da interface \textit{communicator}, implementa-se formas de comunicação e com base na interface \textit{orderer} é possível implementar formas de ordenação. O Hermes original implementou comunicação sockets TCP e ordenação Raft.

\begin{figure}[!htb]
\centering
\caption{Interfaces do código Hermes}
\includegraphics[width=0.8\linewidth]{figures/hermes-ifaces.png}
{\flushleft Fonte - Adaptado de \textcite{renan2021hermes}}
\label{fig:interfaces-hermes}
\end{figure}

Na Figura \ref{fig:interfaces-hermes} é possível ver os dois métodos que precisam ser implementados para poder estender a interface Communicator.  O método \textit{Listen} executa a rotina que recebe as mensagens de aplicações clientes. Cada requisição recebida será submetida à função de tratamento passada por parâmetro no momento da chamada deste método. Além disso, o método \textit{Deliver}, fica responsável por entregar a requisição à aplicação destinada. Ele retorna uma cadeia de bytes para permitir que uma implementação retorne a resposta do servidor ao qual a mensagem foi entregue \cite{renan2021hermes}. No caso de protocolo HTTP, é necessário que a requisição tenha que ter seu HOST alterado para poder ser redirecionada ao servidor.

O \textit{HTTPCommunicator} recebe uma configuração básica: endereço para o recebimento de mensagens; endereço para entrega das mensagens; número de tentativas de conexão com o serviço que receberá as mensagens ordenadas; o tempo entre uma tentativa de conexão e outra e o tamanho dos \textit{buffers} de de armazenamento das mensagens recebidas dos clientes e das respostas do serviço \cite{renan2021hermes}.
